...
import { citiesData } from "../DataSet/citiesData";
...
export default function CitiesOptions() {
  ...
  const handleShowMore = () => {
    setVisibleCount(prev => Math.min(prev + 12, citiesData.length));
  };
  ...
  const visibleCities = citiesData.slice(0, visibleCount);
  const hasMore = visibleCount < citiesData.length;
